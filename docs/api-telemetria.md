...
public function store(StoreRidesRequest $request)
{
    ...
    DB::transaction(function () use ($data, $user, &$accepted) {
        foreach ($data['rides'] as $ride) {         // już posortowane rosnąco
            if ($ride['seq'] !== $accepted + 1 && $ride['seq'] > $accepted) {
                break;                              // dziura w numeracji — dalej nie idziemy
            }
...
Urządzenie samo nie wysyła przesyłek z dziurami, ale serwer nie powinien na tym
polegać: potwierdzenie numeru, którego nie ma w bazie, kasuje przejazd
z urządzenia bezpowrotnie.
