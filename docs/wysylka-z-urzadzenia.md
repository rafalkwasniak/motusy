...
| linia punktu krótsza niż cztery liczby po przecinku | **piąte i dalsze pola są pomijane, nie odrzucane** — format jest pozycyjny (§8) i rozszerzenie linii nie zepsuje wysyłki |
...
**Nowe pola w linii punktu można dokładać bez uzgodnienia z serwerem.** Piąte
i dalsze są dziś pomijane, więc firmware wysyłający `dlon,dlat,dt,lean,acc`
dostanie 200, a nie 422 — portal zacznie z nich korzystać, gdy będzie wiedział,
co znaczą. Odwrotna kolejność (najpierw serwer, potem firmware) nie działa:
pola, którego nie ma, nie da się odczytać wstecz z już wysłanych śladów.
